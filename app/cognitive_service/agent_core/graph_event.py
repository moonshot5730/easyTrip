...
def handle_streaming_event(event: dict):
    ...
    match event_type:
        ...
        case ToolEvents.START:
            ...
            yield format_sse_event_state_json(
                tag_name=SSETag.TOOL,
                event_type=ToolEvents.START,
                name=tool_name,
                status="시작",
            )
        ...
        case ToolEvents.END:
            ...
            if node_name == "tavily_web_search":
                parsed_markdown_search_result = parse_tavily_results_markdown(
                    tool_output
                )
        ...
        case RetrieverEvents.START:
            ...
            yield format_sse_event_state_json(
                tag_name=SSETag.NODE,
                event_type=RetrieverEvents.START,
                name=node_name,
                status="시작",
            )
...
def format_sse_event_state(
    tag_name: str, event_type: str, name: str, status: str
) -> str:
    """
    SSE 마크다운 메시지 생성기.
    """
    return textwrap.dedent(
        f"""{tag_name} ### {event_type}  
                            - 정보: `{name}`  
                            - 상태: {status}"""
    )


def format_sse_event_state_json(
    tag_name: str, event_type: str, name: str, status: str
) -> str:
    payload = {"event_type": event_type, "name": name, "status": status}
