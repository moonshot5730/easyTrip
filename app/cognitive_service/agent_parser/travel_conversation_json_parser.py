...
from app.cognitive_service.agent_core.graph_state import AgentState, get_latest_messages
...
extract_travel_info_prompt = PromptTemplate.from_template(textwrap.dedent("""
...
사용자 메시지: {user_query}""")).partial(format_instructions=travel_info_parser.get_format_instructions())
...
def extract_info_llm_parser(state: AgentState):
    ...
    logger.info(f"[extract_info_llm_parser START!] 🧾 전송한 프롬프트 정보: {formatted_prompt}\n원본 LLM 응답:\n {raw_output.content}")
